...
// 根据最前方的引用获取摘要
let summaryStartIdxCollect=[]
function getAppropriateSummary(content,summaryLens,summaryStartIdx=0,boundaryLimitIdx=300){
  summaryStartIdxCollect.push(summaryStartIdx)
  let [minLen,maxLen]=summaryLens
  let htmlContent=md2Html(content)
  let realHtmlContent=htmlContent.substr(summaryStartIdx)

  let pattern=/<blockquote>([\s\S]*?)<\/blockquote>/
  let match=realHtmlContent.match(pattern)
  // 找不到合适的，返回默认
  if(!match || match.index>boundaryLimitIdx ){
    // 处理md
    let summary=content.substr(summaryStartIdxCollect[0],maxLen)+'...'
      // .replace(/-{3,}/,'')  // 清除 hr
  ...
  if(match[1].length<minLen){
    return getAppropriateSummary(content,summaryLens,match.index+match[1].length,boundaryLimitIdx)
  }
